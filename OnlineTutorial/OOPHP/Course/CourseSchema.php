...
class CSchema extends Database{
	...
	function Schema(){
		...
		{	
				...
				$sql0="select dept_id from Faculty where username='$uname'";
				$res0=mysqli_query($this->connection,$sql0);
				while ($row=mysqli_fetch_array($res0)) {
					$DID=$row['dept_id'];
				}

				$sql1="INSERT into Course (CID,dept_id,CName,COutcome,CDuration,CPrereq,Last_Modified) values ('','$DID','$coursename','$outcome','$duration','$requisit',now())";	
